...
// PATCH CATHY PETS V23 â Logo Orange + Module Animaux
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 600 120" style="max-width:420px;width:100%;height:auto">
  ...
  <text x="212" y="105" font-family="'Segoe UI',Arial,sans-serif" font-size="10" fill="#FF6B00" opacity="0.6">SAS Â· SIRET 928 564 269 Â· cathypetsitter.com Â· 07.81.30.41.10</text>
</svg>`;
...
window.ouvrirModalAnimal = function(contexte) {
  ...
  if (typeof data !== 'undefined' && data.animaux && data.animaux.length > 0) {
    animauxExistants = data.animaux.map((a, i) =>
      `<option value="${i}">${a.nom || '?'} â ${a.espece || '?'} ${a.race ? '(' + a.race + ')' : ''}</option>`
    ).join('');
  }
  ...
  modal.innerHTML = `
    <div class="modal-animal">
      <h3>ð¾ Ajouter un animal</h3>
      ${animauxExistants ? `
        <label>ð SÃ©lectionner un animal existant</label>
        <select id="modal-animal-existant" onchange="remplirAnimalExistant(this.value)">
          <option value="">â Saisie manuelle â</option>
          ${animauxExistants}
        </select>
        <div style="text-align:center;color:#64748b;margin:10px 0;font-size:12px">â ou saisir manuellement â</div>
      ` : ''}
      ...
      <div class="modal-animal-row">
        <div>
          ...
          <select id="modal-animal-espece">
            ...
            <option value="ð Chien">ð Chien</option>
            <option value="ð Chat">ð Chat</option>
            <option value="ð¦ NAC">ð¦ NAC</option>
            <option value="ð¦ Oiseau">ð¦ Oiseau</option>
            <option value="ð¦ Reptile">ð¦ Reptile</option>
            <option value="ð¹ Rongeur">ð¹ Rongeur</option>
            <option value="ð¾ Autre">ð¾ Autre</option>
          </select>
        </div>
        <div>
          <label>Race</label>
      ...
      <div class="modal-animal-row">
        ...
        <div>
          <label>NÂ° puce / tatouage</label>
          <input type="text" id="modal-animal-puce" placeholder="NÂ° identification">
        </div>
      </div>
      <label>Carnet de vaccinations</label>
      <select id="modal-animal-vaccin">
        <option value="oui">â Oui â Ã  jour</option>
        <option value="non">â Non</option>
        <option value="inconnu">â Non renseignÃ©</option>
      </select>
      <label>CaractÃ¨re / Remarques</label>
      ...
      <div class="modal-animal-actions">
        ...
        <button type="button" class="btn-confirm-animal" onclick="confirmerAjoutAnimal('${contexte}')">â Ajouter</button>
      </div>
    </div>`;
...
window.remplirAnimalExistant = function(index) {
  ...
  const especeMap = { chien:'ð Chien', chat:'ð Chat', nac:'ð¦ NAC', oiseau:'ð¦ Oiseau', reptile:'ð¦ Reptile', rongeur:'ð¹ Rongeur' };
...
window.confirmerAjoutAnimal = function(contexte) {
  ...
  if (!espece) { alert("L'espÃ¨ce est obligatoire"); return; }
...
function rafraichirListeAnimaux(contexte) {
  ...
  if (!animaux.length) {
    c.innerHTML = '<div style="color:#64748b;font-size:13px;text-align:center;padding:12px">Aucun animal â cliquez â</div>';
    return;
  }
  c.innerHTML = animaux.map(a => `
    <div class="animal-card">
      <div class="animal-card-info">
        ...
        <span>Vaccins: <strong>${a.vaccin === 'oui' ? 'â' : a.vaccin === 'non' ? 'â' : 'â'}</strong></span>
        ${a.remarques ? `<span style="min-width:100%;color:#fbbf24;font-size:12px">â ï¸ ${a.remarques}</span>` : ''}
      </div>
      <button class="btn-remove-animal" onclick="supprimerAnimalDuContexte('${contexte}',${a.id})" title="Retirer">â</button>
    </div>`).join('');
}
...
window.ajouterLogoPDF = function(doc, x, y, w) {
  ...
  doc.setFontSize(7);
  doc.text('SAS Â· SIRET 928 564 269 Â· cathypetsitter.com Â· 07.81.30.41.10', x, y + 23);
...
window.ajouterAnimauxPDF = function(doc, contexte, startY) {
  ...
  doc.text('Animaux concernÃ©s', 15, y); y += 6;
  doc.setDrawColor(255,107,0); doc.setLineWidth(0.5); doc.line(15,y,195,y); y += 5;
  doc.setFontSize(8); doc.setFont('helvetica','bold'); doc.setTextColor(100,100,100);
  doc.text('Nom',16,y); doc.text('EspÃ¨ce',48,y); doc.text('Race',80,y);
  doc.text('Poids',115,y); doc.text('NÂ° Puce',133,y); doc.text('Vaccins',168,y); y += 5;
  doc.setFont('helvetica','normal'); doc.setTextColor(40,40,40);
  animaux.forEach(a => {
    doc.setFontSize(9);
    doc.text(a.nom||'â',16,y); doc.text((a.espece||'').replace(/^.{2} /,''),48,y);
    doc.text(a.race||'â',80,y); doc.text(a.poids?a.poids+' kg':'â',115,y);
    doc.text(a.puce||'â',133,y); doc.text(a.vaccin==='oui'?'OK':a.vaccin==='non'?'NON':'?',168,y);
    if (a.remarques) { y+=4; doc.setFontSize(7); doc.setTextColor(180,120,0); doc.text('  â '+a.remarques.substring(0,80),16,y); doc.setTextColor(40,40,40); }
    y += 6;
  });
  return y + 3;
};
...
function creerModuleAnimaux(contexte, titre) {
  return `<div class="animaux-module" id="animaux-module-${contexte}">
    ...
    <div class="animaux-list" id="animaux-list-${contexte}">
      <div style="color:#64748b;font-size:13px;text-align:center;padding:12px">Aucun animal â cliquez â</div>
    </div>
  </div>`;
}
...
function injecterDansSection(texteRecherche, logoId, contexte, titreAnimaux) {
  ...
  // MÃ©thode 1 : cherche dans les titres
  allElements.forEach(el => {
    if (el.textContent.includes(texteRecherche)) cible = el;
  });

  // MÃ©thode 2 : cherche dans tout le body par texte
  if (!cible) {
  ...
  // InsÃ¨re le logo aprÃ¨s le titre
  ...
  if (ancienAnimaux && !ancienAnimaux.id.startsWith('animaux-module-')) {
    ...
    // Pas trouvÃ© d'ancien bloc â insÃ¨re aprÃ¨s le logo
...
function initPatch() {
  console.log('ð¾ Patch CathyPets V23 â Chargement...');

  // Injection dans les 3 sections
  const r1 = injecterDansSection('Nouveau devis', 'logo-devis', 'devis', 'Animaux concernÃ©s');
  const r2 = injecterDansSection('Facture Transport', 'logo-facture-transport', 'factureTransport', 'Animaux transportÃ©s');
  const r3 = injecterDansSection('bon de transport', 'logo-bon-transport', 'bonTransport', 'Animaux transportÃ©s');

  // Si l'injection auto n'a pas marchÃ© pour tout, on cherche plus large
  if (!r1 || !r2 || !r3) {
    // Fallback : cherche les textes "Animaux concernÃ©s" / "Animal(aux)" et insÃ¨re Ã  cÃ´tÃ©
    document.querySelectorAll('label, h4, h3, span, div').forEach(el => {
      const txt = el.textContent.trim();
      if (txt.includes('Animaux concernÃ©s') && !document.getElementById('animaux-module-devis')) {
        el.closest('div')?.insertAdjacentHTML('afterend', creerModuleAnimaux('devis', 'Animaux concernÃ©s'));
      }
      if (txt.includes('Animal(aux)') && !document.getElementById('animaux-module-factureTransport')) {
        el.closest('div')?.insertAdjacentHTML('afterend', creerModuleAnimaux('factureTransport', 'Animaux transportÃ©s'));
      }
      if (txt.includes('Animaux transportÃ©s') && !document.getElementById('animaux-module-bonTransport')) {
        el.closest('div')?.insertAdjacentHTML('afterend', creerModuleAnimaux('bonTransport', 'Animaux transportÃ©s'));
      }
    });
  }

  console.log('â Patch CathyPets V23 â OK !');
}

// Lancer dÃ¨s que le DOM est prÃªt
